...
| `use skill <name>` | Load domain expertise (native syntax) |
...
Pre-built expertise packages. OpenCode discovers these natively from `.opencode/skill/`:

```bash
use skill testing-patterns    # Load Feathers' dependency-breaking techniques
use skill swarm-coordination  # Multi-agent orchestration patterns
```
