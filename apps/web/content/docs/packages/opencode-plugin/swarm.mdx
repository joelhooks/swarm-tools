...
| `bead_id` | string | ✅ | Subtask cell ID |
...
2. **Runs UBS scan** - Scans `files_touched` for bugs (null safety, XSS, async/await issues)
3. **Releases reservations** - Auto-releases all file locks held by the worker
4. **Records learning signals** - Tracks duration, errors, retries for pattern learning
5. **Closes the cell** - Updates cell status to closed with completion reason

**Why Not `hive_close` Directly?**

Direct `hive_close` skips critical cleanup:
- Reservations stay locked (blocks other agents)
- No bug scanning (security/quality issues slip through)
