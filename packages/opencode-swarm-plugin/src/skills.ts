 ...
 *
 * Discovery location:
 * 1. {projectDir}/.opencode/skill/ (OpenCode native skills)
 *
 * Skill format:
 * ```markdown
 * ---
 ...
 *
 * OpenCode native skills only - discovered from .opencode/skill/
 */
const PROJECT_SKILL_DIRECTORIES = [".opencode/skill"] as const;
...
/**
 * Discover all skills in project directory
 *
 * OpenCode native skills only - discovered from .opencode/skill/
 */
...
- Tags for discoverability`,
  args: {
    ...
    directory: tool.schema
      .enum([".opencode/skill"])
      .optional()
      .describe("Where to create skill (default: .opencode/skill)"),
  },
  async execute(args) {
    ...
    const skillDir = join(skillsProjectDirectory, baseDir, args.name);
    ...
    try {
      ...
      const response: Record<string, unknown> = {
        ...
        skill: args.name,
        path: skillPath,
        message: `Created skill '${args.name}'. It's now discoverable via OpenCode native skills.`,
        next_steps: [
...
### scripts/
Executable scripts for automation. These can be run from skill directory.

### references/
Documentation for additional context and details.
...
with native OpenCode skill syntax when relevant to subtasks:
